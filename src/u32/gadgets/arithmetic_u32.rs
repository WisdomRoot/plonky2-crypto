    ...
    for CircuitBuilder<F, D>
{
    ...
    // Returns x * y + z.
    // X, Y, Z goldilocks: 64-bit
    // x -> X [0000 xxxx] <= 2^32-1
    // y -> Y [0000 xxxx] <= 2^32-1
    // z -> Z [0000 xxxx] <= 2^32-1
    // x*y -> X*Y [bbbb bbbb] <= 2^64 - 2*2^32 + 1
    // (x*y) + z  [cccc cccc] <= 2^64 - 2^32 < 2^64 - 2^32 - 1 = p goldilocks prime
    fn mul_add_u32(&mut self, x: U32Target, y: U32Target, z: U32Target) -> (U32Target, U32Target) {
